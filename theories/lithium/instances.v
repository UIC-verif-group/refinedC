...
(** * Simplification *)
Lemma simplify_hyp_id {Σ} M (P T : iProp Σ) :
  simplify_hyp P M T :- return T.
Proof. iIntros "HT Hl". iModIntro. iFrame. Qed.
Definition simplify_hyp_id_inst Σ M (P : iProp Σ) :=
  [instance simplify_hyp_id M P as SimplifyHyp P M None].
Global Existing Instance simplify_hyp_id_inst | 100.

Lemma simplify_goal_id {Σ} M (P : iProp Σ) T :
  simplify_goal M P T :- exhale P; return T.
Proof. by iIntros "$ !>". Qed.
Definition simplify_goal_id_inst Σ M (P : iProp Σ) :=
  [instance simplify_goal_id M P as SimplifyGoal M P None].
Global Existing Instance simplify_goal_id_inst | 100.

(** * Subsumption *)
Lemma subsume_id {Σ A M} (P : iProp Σ) (T : A → iProp Σ):
  subsume P M (λ _, P) T :- ∃ x, return T x.
Proof. iIntros "[% ?] $ !>". by iExists _. Qed.
Definition subsume_id_inst := [instance @subsume_id].
Global Existing Instance subsume_id_inst | 1.

Lemma subsume_simplify {Σ A} (P1 : iProp Σ) M (P2 : A → iProp Σ) o1 o2 T :
  (* TCOneIsSome must be first here since [instance ...] reverse the order *)
  ∀ `{!TCOneIsSome o1 o2} {SH : SimplifyHyp P1 M o1} {SG : ∀ x, SimplifyGoal M (P2 x) o2},
    let GH := (SH (∃ x, P2 x ∗ T x)%I).(i2p_P) in
    let GG := (P1 -∗ ‖M‖ ∃ x, (SG x (T x)).(i2p_P))%I in
    let G :=
       match o1, o2 with
    ...
    subsume P1 M P2 T :- return G.
Proof.
  iIntros (???) "/= Hs Hl".
  destruct o1 as [n1|], o2 as [n2|] => //. 1: case_match.
  1,3,4: by iDestruct (i2p_proof with "Hs Hl") as "Hsub".
  all: iMod ("Hs" with "Hl") as "[% HSG]".
  all: iDestruct (i2p_proof with "HSG") as ">[$ $]"; by iModIntro.
Qed.
...
Global Instance sep_list_related_to Σ A B id ig l f :
  RelatedTo (Σ:=Σ) (λ x : B, sep_list id A (ig x) (l x) (f x)) :=
  {| rt_fic := FindSepList id |}.

Lemma find_sep_list {Σ} id (T : _ → iProp Σ):
  find_in_context (FindSepList id) T :-
    pattern: A ig l f, sep_list id A ig l f; return T (sep_list id A ig l f).
Proof. liFromSyntax. iIntros "(%&%&%&%&?&?)". iExists (sep_list _ _ _ _ _). iFrame. Qed.
Definition find_sep_list_inst := [instance @find_sep_list with FICSyntactic].
Global Existing Instance find_sep_list_inst | 1.

Lemma subsume_sep_list_eq {Σ B M} A id ig (l1 : list A) (l2 : B → list A) f (T : B → iProp Σ) :
  subsume (sep_list id A ig l1 f) M (λ x : B, sep_list id A ig (l2 x) f) T :-
...
Lemma subsume_sep_list_insert_in_ig {Σ B M} A id ig i x (l1 : list A) (l2 : B → list A)
  (f : nat → A → iProp Σ) (T : B → iProp Σ) :
  subsume (sep_list id A ig (<[i := x]>l1) f) M (λ x : B, sep_list id A ig (l2 x) f) T
     where `{!CanSolve (i ∈ ig)} :-
  return subsume (sep_list id A ig l1 f) M (λ x : B, sep_list id A ig (l2 x) f) T.
Proof.
...
Lemma subsume_sep_list_insert_not_in_ig {Σ M} A B id ig i x (l1 : list A) l2 (f : nat → A → iProp Σ) (T : B → iProp Σ) :
  subsume (sep_list id A ig (<[i := x]>l1) f) M (λ x : B, sep_list id A ig (l2 x) f) T
    where `{!CanSolve (i ∉ ig)} :-
      exhale ⌜i < length l1⌝%nat;
      inhale f i x;
      y ← (sep_list id A (i :: ig) l1 f) :‖M‖> (λ x : B, sep_list id A (i :: ig) (l2 x) f);
...
Proof.
  ...
  iMod ("Hsub" with "Hx [Hl]") as "[% [[%Heq Hl] [% [% [? HT]]]]]". {
...
Lemma subsume_sep_list_trivial_eq {Σ M} A B id ig (l : list A) (f : nat → A → iProp Σ) (T : B → iProp Σ) :
  subsume (sep_list id A ig l f) M (λ x : B, sep_list id A ig l f) T :- ∃ x, return T x.
...
Lemma subsume_sep_list_cons {Σ M} A B id ig (x1 : A) (l1 : list A) l2 (f : nat → A → iProp Σ) (T : B → iProp Σ) :
  subsume (sep_list id A ig (x1 :: l1) f) M (λ y : B, sep_list id A ig (l2 y) f) T :-
    exhale ⌜0 ∉ ig⌝;
    ∀ id', inhale (f 0%nat x1);
    inhale (sep_list id' A (pred <$> ig) l1 (λ i, f (S i)));
    ‖M‖ ∃ y x2 l2', exhale ⌜l2 y = x2 :: l2'⌝;
    exhale (f 0%nat x2);
...
Proof.
  ...
  iDestruct ("Hs" $! {|sep_list_len := _|} with "H0 [H]") as ">Hs".
  ...
  iDestruct "Hs" as (??? Heq1) "[? [[%Heq2 H] ?]]". iModIntro.
