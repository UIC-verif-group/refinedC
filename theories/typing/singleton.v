...
Section value.
  ...
  Lemma value_simplify v ot p M T:
    (⌜v = p⌝ -∗ ⌜v `has_layout_val` ot_layout ot⌝ -∗ ⌜mem_cast_id v ot⌝ -∗ ‖M‖ T)
    ⊢ simplify_hyp (v ◁ᵥ value ot p) M T.
  ...
  Lemma value_subsume_goal M A v v' ly ty T:
    (⌜ty.(ty_has_op_type) ly MCId⌝ ∗ (v ◁ᵥ ty -∗ ‖M‖ ∃ x, ⌜v = v' x⌝ ∗ T x))
    ⊢ subsume (v ◁ᵥ ty) M (λ x : A, v ◁ᵥ value ly (v' x)) T.
  Proof.
    iIntros "[% HT] Hty". iDestruct (ty_size_eq with "Hty") as %Hly; [done|].
    iDestruct (ty_memcast_compat_id with "Hty") as %?; [done|].
    iMod ("HT" with "Hty") as (? ->) "?". iModIntro. by iFrame.
  Qed.
  Definition value_subsume_goal_inst := [instance value_subsume_goal].
  Global Existing Instance value_subsume_goal_inst.

  Lemma value_subsume_goal_loc A M l v' ot ty T:
    (⌜ty.(ty_has_op_type) ot MCId⌝ ∗ ∀ v, v ◁ᵥ ty -∗ ‖M‖ ∃ x, ⌜v = (v' x)⌝ ∗ T x)
    ⊢ subsume (l ◁ₗ ty) M (λ x : A, l ◁ₗ value ot (v' x)) T.
  Proof.
    iIntros "[% HT] Hty".
  ...
  Lemma value_subsume_own_ptrop A M l β (v' : A → val) ty T:
    (l ◁ₗ{β} ty -∗ ‖M‖ ∃ x, ⌜v' x = l⌝ ∗ T x)
    ⊢ subsume (l ◁ₗ{β} ty) M (λ x : A, l ◁ᵥ value PtrOp (v' x)) T.
  Proof.
    iIntros "HT Hty". iMod ("HT" with "Hty") as (? Heq) "?". iModIntro. iFrame.
  ...
  Lemma value_merge v l ot M T:
    find_in_context (FindVal v) (λ ty:type, ⌜ty.(ty_has_op_type) (UntypedOp (ot_layout ot)) MCNone⌝ ∗ (l ◁ₗ ty -∗ ‖M‖ T))
    ⊢ simplify_hyp (l ◁ₗ value ot v) M T.
  Proof.
...
Section at_value.
  ...
  Lemma at_value_simplify_hyp_val v v' ty M T:
    (v ◁ᵥ value PtrOp v' -∗ v' ◁ᵥ ty -∗ ‖M‖ T)
    ⊢ simplify_hyp (v ◁ᵥ at_value v' ty) M T.
  ...
  Lemma at_value_simplify_goal_val v v' ty M T:
    v ◁ᵥ value PtrOp v' ∗ v' ◁ᵥ ty ∗ T
    ⊢ simplify_goal M (v ◁ᵥ at_value v' ty) T.
  ...
  Lemma at_value_simplify_hyp_loc l v' ty M T:
    (l ◁ₗ value PtrOp v' -∗ v' ◁ᵥ ty -∗ ‖M‖ T)
    ⊢ simplify_hyp (l ◁ₗ at_value v' ty) M T.
  Proof. iIntros "HT [??]". by iApply ("HT" with "[$] [$]"). Qed.
  ...
  Lemma at_value_simplify_goal_loc l v' ty M T:
    l ◁ₗ value PtrOp v' ∗ v' ◁ᵥ ty ∗ T
    ⊢ simplify_goal M (l ◁ₗ at_value v' ty) T.
  Proof. by iIntros "[$ [$ $]] !>". Qed.
...
Section place.
  ...
  Lemma place_simplify l β p M T:
    (⌜l = p⌝ -∗ ‖M‖ T)
    ⊢ simplify_hyp (l◁ₗ{β} place p) M T.
  Proof. iIntros "HT ->". by iApply "HT". Qed.
  Definition place_simplify_inst := [instance place_simplify with 0%N].
  Global Existing Instance place_simplify_inst.

  Lemma place_simplify_goal l β p M T:
    ⌜l = p⌝ ∗ T
    ⊢ simplify_goal M (l◁ₗ{β} place p) T.
  Proof. by iIntros "[-> $] !>". Qed.
  ...
  Lemma simplify_goal_ex_place l β ty M T:
    simplify_goal M (l ◁ₗ{β} ty) T :- exhale ⌜ty = place l⌝; return T.
  ...
  Proof.
    iIntros "SH" (Φ) "Hl HΦ".
    iApply place_to_wp_fupd. { iIntros ">$". }
    iMod (i2p_proof with "SH Hl") as ([β2 ty2]) "[Hl HP]".
    iModIntro.
    iApply ("HP" with "Hl").
  ...
  Lemma typed_read_end_simpl E l β ty ly n mc {SH:SimplifyHyp (l ◁ₗ{β} ty) (={E}=) (Some n)} a T:
    (SH (find_in_context (FindLoc l) (λ '(β2, ty2),
  ...
  Proof.
    ...
    iMod (i2p_proof with "SH Hl") as ([β2 ty2]) "[Hl HP]" => /=.
    iModIntro. iExists _, _, True%I. iFrame. iSplit; [done|].
  ...
  Lemma typed_write_end_simpl b E ot v ty1 l β ty2 n {SH:SimplifyHyp (l ◁ₗ{β} ty2) (={E}=) (Some n)} T:
  ...
  Proof.
    ...
    iMod (i2p_proof with "SH Hl") as ([β2' ty2']) "[Hl HP]" => /=.
    iModIntro. iExists _, _, _, True%I. iFrame. iSplit; [done|].
...
Global Hint Extern 99 (SimplifyGoal _ (_ ◁ₗ{_} _.1ₗ) _) =>
